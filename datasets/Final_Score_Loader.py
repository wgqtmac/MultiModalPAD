...
# class TestImgLoader(data.Dataset):
...
class FinalImgLoader(data.Dataset):
    def __init__(self, root_folder, list_file, transform=None, loader1=RGB_loader, loader2=Gray_loader, loader3=YCbCr_loader, stage='Train'):
        self.root_folder = root_folder
        self.loader1 = loader1
        self.loader2 = loader2
        self.transform = transform
        ...
        fp_items = [line.rstrip('\n').split(' ') for line in open(list_file)]
        for file in fp_items:
            if os.path.isfile(os.path.join(root_folder, file[0])) and \
            os.path.isfile(os.path.join(root_folder, file[1])) and \
            os.path.isfile(os.path.join(root_folder, file[2])):
                tup = (file[0], file[1], file[2])
                items.append(tup)
        self.items = items
        print('\nStage: ' + stage)
        print('The number of samples: {}'.format(len(fp_items)))
    ...
    def __getitem__(self, index):
        image1, image2, image3 = self.items[index]
        img1 = self.loader1(os.path.join(self.root_folder, image1))
        img2 = self.loader2(os.path.join(self.root_folder, image2))
        img3 = self.loader2(os.path.join(self.root_folder, image3))
        if self.transform is not None:
...
if __name__ == '__main__':
    ...
    src_dataset = FinalImgLoader(params.root_folder, os.path.join(params.root_folder, params.src_train_list),
                            transforms.Compose([
